...
class WhatsAppService:
    ...
    def _build_template_payload(self, template_name: str, template_data: dict, to_phone: str, company_id: str) -> dict:
        ...
        if is_etd_company and name.startswith("etd_"):
            ...
            Reemplazar desde la línea 2612 (después del bloque ETD) hasta la línea 2716
            """
            ...
            if name_normalized == "agendar_llamada":
                ...
                # Body: {{1}} = first_name
                # Button URL: {{1}} = deal_id
                logger.info("[BUILD_TEMPLATE] Detectado template: followup_missed_calls")
                first_name = td.get("first_name") or ""
                deal_id = td.get("deal_id") or ""
                ...
                logger.warning("[BUILD_TEMPLATE] ⚠️ llamada_perdida requiere es_ES")
                first_name = td.get("first_name") or ""
                deal_id = td.get("deal_id") or ""
                ...
                components.append({
                    "type": "button",
                    "sub_type": "url",
                    "index": 1,  # Es el segundo botón (el primero es QUICK_REPLY)
                    "parameters": [
                        {"type": "text", "text": deal_id}
                    ]
                })
    ...
    def save_media_message(
        ...
        *,
        ...
        Args:
            ...
            status: estado a guardar (por defecto infiere: in→"media_received", out→"media_sent").
        """
        try:
            dbm = self.db_manager
            ...
            if direction == "in":
                raw_phone = (
                    ...
                    or (
                        msg.get("contacts", [{}])[0].get("wa_id")
                        if isinstance(msg.get("contacts"), list) and msg.get("contacts")
                        else None
                    )
                )
            else:
            ...
            #     Preferimos SIEMPRE el id real del mensaje de WhatsApp
            raw_wa_id = (
                msg.get("id")
                or msg.get("wamid")
            ...
            if status is None:
                # Queremos que medias entrantes queden como 'media_received'
            ...
            params = [
                ...
                clean_phone,
                responsible_email or "",
                msg_uid,          # 👈 AHORA usamos msg_uid, NO el teléfono ni wa_id tal cual
                from_me,
                status,
                chat_url,
                str(chat_id),
                            ...
                            try:
                                ...
                                if file_service:
                                    try:
                                        ...
                                        try:
                                            message_service.save_media_message(msg, wa_id, file_info, company_id=company_id, direction="in", status="media_received")
                                        except TypeError:
